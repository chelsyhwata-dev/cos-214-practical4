...
void HighPriorityDecorator::display(int depth) const {
    // Print our own extra marker line, then let the wrapped item (or the next decorator down the stack) print itself as normal. This is how stacked decorators each get to add their own line without needing to know about each other.
